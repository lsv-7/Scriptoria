...
def _run_async_generation(id, user_id):
    """
    Background worker thread to compile the story package.
    """
    try:
        import datetime
        from backend.services.firebase_service import firebase_service
...
def generate_all_preproduction(id):
    """
    Spawns a background thread to generate all 9 pre-production modules.
    Returns immediately with status "processing" to prevent HTTP request timeouts.
    """
    user_id = request.current_user.get("uid")
    project = firebase_service.get_document("projects", id)
    if not project:
    ...
    import threading
    thread = threading.Thread(target=_run_async_generation, args=(id, user_id))
    thread.start()
    
    return success_response({"status": "processing"}, "Pre-production package generation started in background.", 202)

@project_bp.route("/project/<id>/generation-status", methods=["GET"])
@login_required
def get_generation_status(id):
    """Checks the existence of all 9 pre-production modules to compute progress."""
    user_id = request.current_user.get("uid")
    project = firebase_service.get_document("projects", id)
    if not project:
        return error_response("Project not found.", 404)
